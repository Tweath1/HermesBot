        ...
        if message.content.lower().startswith("$build"):
            ...
            if len(m) < 3:
                ...
                if actgod in Assassins:
                    ...
                    color = "fc0303"
                embed = discord.Embed(title=f"{actgod} {role} Build",
                                      description="Games: {} | Wins: {} | WR: {} \n [See more info here](https://www.smitestats.gg/#/{})".format(
                                          data["games"], data["wins"], data["winRate"], actgod.replace(" ", "_")),
        ...
        if message.content.lower().startswith("$matchups"):
            ...
            if len(m) < 3:
                ...
                if actgod in Assassins:
                    ...
                    color = "fc0303"
                embed = discord.Embed(title=actgod + " " + role + " Worst Matchups",
                                      description="Games: {} | Wins: {} | WR: {} \n [See more info here](https://www.smitestats.gg/#/{})".format(
                                          data["games"], data["wins"], data["winRate"], actgod.replace(" ", "_")),
                                      color=int(color, base=16))
                embed.set_thumbnail(url=iconURL)
                for i, matchup in enumerate(data):
                    if matchup not in ["games", "wins", "winRate"] and i < 6:
                        embed.add_field(name=matchup,
                                        value="Games Played: " + str(data[matchup]["timesPlayed"]) + "\nWR: " + str(
                                            data[matchup]["winRate"]) + "%", inline=True)
                await message.channel.send(embed=embed)
        ...
        if message.content.lower().startswith("$paths"):
            ...
            if len(m) < 3:
                ...
                if actgod in Assassins:
                    ...
                    color = "fc0303"
                embed = discord.Embed(title=actgod + " " + role + " Build Paths",
                                      description="[See more info here](https://www.smitestats.gg/#/{})".format(
                                          actgod.replace(" ", "_")), color=int(color, base=16))
                embed.set_thumbnail(url=iconURL)
                for i, path in enumerate(data):
                    if i < 2:
                        games = data[path]["wins"] + data[path]["losses"]
                        embed.add_field(name=f"Path {i + 1}",
                                        value="{}\nGames: {} Win Rate: {}".format(path.replace(",", ", "), games, round(
