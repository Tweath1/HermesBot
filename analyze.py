...
def get_top_builds(client, god, role, patch, rank="All Ranks"):
    ...
    return {**sort_top_dict(dict(top_dict), client),
...
def get_god_stats(client, god, level):
    ...
    ret_stats = {
        ...
        "MagicalPower": round(base_stats["MagicalPower"] + (per_level_stats["MagicalPowerPerLevel"] * level), 2) * (
                    1 / 5),
        ...
        "PhysicalProtection": round(
            base_stats["PhysicalProtection"] + (per_level_stats["PhysicalProtectionPerLevel"] * level)),
        ...
        for side in ret_data["levelDiff"]:
            for role in ret_data["levelDiff"][side]:
                if side == "Winner":
                    ...
                    opp_side = "Winner"
                ret_data["levelDiff"][side][role]["level_diff"] = ret_data["levelDiff"][side][role]["level"] - \
                                                                  ret_data["levelDiff"][opp_side][role]["level"]
        for key in match:
            if "player" in key and ret_data["killPart"][match[key]["Win_Status"]]["totalKills"] <= 0:
                ...
                ret_data["killPart"][match[key]["Win_Status"]][match[key]["Role"]] = {
                    "kills": match[key]["Kills_Player"], "assists": match[key]["Assists"],
                    "killShare": round((match[key]["Kills_Player"] + match[key]["Assists"]) /
                                       ret_data["killPart"][match[key]["Win_Status"]]["totalKills"] * 100, 2)}

            if "player" in key:
                ret_data["goldScore"][match[key]["Win_Status"]][match[key]["Role"]] = {
                    "gold": match[key]["Gold_Earned"],
                    "goldShare": round(
                        match[key]["Gold_Earned"] / ret_data["goldScore"][match[key]["Win_Status"]]["totalGold"] * 100,
                        2)}
                ret_data["damageScore"][match[key]["Win_Status"]][match[key]["Role"]] = {
                    "damage": match[key]["Damage_Player"],
                    "damageShare": round(
                        match[key]["Damage_Player"] / ret_data["damageScore"][match[key]["Win_Status"]][
                            "totalDamage"] * 100, 2)}
    return ret_data
    ...
    if check_roles(match_roles):
        for team in kill_part:
            for role in kill_part[team]:
                if role in roles:
                    ret_data[team][role] = {
                        "efficiency": round(kill_part[team][role]["killShare"] / gold_eff[team][role]["goldShare"], 2)}
    return ret_data
